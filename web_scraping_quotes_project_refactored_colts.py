...
"""
 One additional edit could be creating a print_hint() function. Colt's not 100% sold but here it is:
...
def print_hint(quote, remaining_guesses):  # !! Problem is that we have to pass QUOTE since we don't have access to it anymore
    if remaining_guesses == 3:
        res = requests.get(f"{BASE_URL}{quote['bio-link']}")
        soup = BeautifulSoup(res.text, "html.parser")
        #print(soup.body) for testing
